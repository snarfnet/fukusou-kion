# VoiceprintNFT App Store Submission
...
Base price: 100 JPY in Japan.

Use App Store Connect to generate equivalent worldwide pricing from the Japan base price. The app binary cannot set the sale price by itself.
...
The NFT flow is preparation-only inside the app. Users can export a PNG and metadata JSON, then continue minting outside the app through OpenSea or another service. The app does not sell NFTs inside the app and does not include in-app purchases.

The planned base price is 100 JPY in Japan, with equivalent worldwide pricing set in App Store Connect.
