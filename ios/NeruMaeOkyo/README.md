...
設定画面にOpenAI APIキーを入れると、`gpt-4o-mini-tts`で般若心経の読経ボイスを生成します。生成した音声はアプリのDocumentsに住職ごとのMP3として保存され、次回の再生から同梱音源より優先されます。旧バージョンで生成したキャッシュ音声は、新しい同梱音源を邪魔しないよう別名に切り替えています。
