...
struct OnboardingView: View {
    ...
    var body: some View {
        VStack(spacing: 28) {
            Spacer(); Image(systemName: "rectangle.and.hand.point.up.left").font(.system(size: 72)).foregroundStyle(Color.navy)
...
struct PrimaryButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View { configuration.label.font(.headline).frame(maxWidth: .infinity, minHeight: 52).foregroundStyle(.white).background(configuration.isPressed ? Color.navy.opacity(0.7) : Color.navy, in: RoundedRectangle(cornerRadius: 14)) }
}
