...
WORK_DIR = Path(os.environ.get("SIGNING_WORK_DIR", "/tmp/gomi-calendar-signing"))
...
CI_CERT_MARKERS = ("gomicalendar", "gomi calendar", "ごみ日カレンダー")
...
def generate_csr():
    ...
    run(
        [
            ...
            str(CSR_PATH),
            "-subj",
            "/CN=GomiCalendar CI Distribution/O=TokyoNasu/C=JP",
        ]
    )
...
def delete_known_invalid_certificates():
    ...
    for certificate in certificates:
        ...
        is_product_reel_ci_cert = any(marker in names for marker in CI_CERT_MARKERS)
        should_delete = (
            REPLACE_DISTRIBUTION_CERTIFICATE
            or serial in INVALID_SERIALS
            or is_product_reel_ci_cert
