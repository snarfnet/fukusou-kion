...
struct ContentView: View {
    @StateObject private var viewModel = VehicleCounterViewModel()
        ...
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 10) {
                Image(systemName: "car.fill")
                    ...
                    .foregroundStyle(.red)
                VStack(alignment: .leading, spacing: 3) {
                    Text("赤い線を車両が越えると1台として数えます")
                        .font(.system(size: 13, weight: .heavy, design: .rounded))
                    Text("車両の通過を確認し、中央線を越えた台数を記録します。")
                        .font(.system(size: 12, weight: .semibold, design: .rounded))
        ...
        VStack(spacing: 12) {
            ZStack {
                ...
                VStack(spacing: 10) {
                    ...
                    CountBadge(title: "車両", value: viewModel.count, color: .red)
                }
                .padding(16)
            }
            .frame(height: 178)
