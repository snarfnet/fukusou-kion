...
struct ContentView: View {
    ...
    private var currentModeBanner: some View {
        HStack(spacing: 10) {
            Image(systemName: viewModel.selectedDirection == .in ? "arrow.right.circle.fill" : "arrow.left.circle.fill")
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text("現在の計測モード")
                    ...
                    .foregroundStyle(.white.opacity(0.62))
                Text(viewModel.selectedDirection.displayName)
                    .font(.system(size: 24, weight: .black, design: .rounded))
            }

            Spacer()
        }
        .foregroundStyle(viewModel.selectedDirection == .in ? .mint : .orange)
        .padding(.horizontal, 14)
        .padding(.vertical, 12)
        .background(.white.opacity(0.08), in: RoundedRectangle(cornerRadius: 8, style: .continuous))
        .overlay(
            RoundedRectangle(cornerRadius: 8, style: .continuous)
                .stroke((viewModel.selectedDirection == .in ? Color.mint : Color.orange).opacity(0.55), lineWidth: 1)
        )
    }
        ...
        VStack(spacing: 12) {
            ZStack {
                ...
                VStack(spacing: 10) {
                    ...
                    HStack(spacing: 12) {
                        CountBadge(
                            title: "入店",
                            value: viewModel.countIn,
                            color: .mint,
                            isSelected: viewModel.selectedDirection == .in
                        )
                        CountBadge(
                            title: "退店",
                            value: viewModel.countOut,
                            color: .orange,
                            isSelected: viewModel.selectedDirection == .out
                        )
                    }
                }
                .padding(16)
            ...
            HStack(spacing: 12) {
                ManualButton(label: "-1", systemImage: "minus.circle.fill", tint: .red) {
                    viewModel.adjustSelected(amount: -1)
                }
                CountButton(direction: viewModel.selectedDirection) {
                    viewModel.adjustSelected(amount: 1)
                }
    ...
    private var controlDeck: some View {
        HStack(spacing: 10) {
            Button {
                viewModel.selectDirection(.in)
            } label: {
                Label("計測モード 入店", systemImage: "arrow.right")
            }
            .buttonStyle(DirectionModeButtonStyle(tint: .mint, isSelected: viewModel.selectedDirection == .in))

            Button {
                viewModel.selectDirection(.out)
            } label: {
                Label("計測モード 退店", systemImage: "arrow.left")
            }
            .buttonStyle(DirectionModeButtonStyle(tint: .orange, isSelected: viewModel.selectedDirection == .out))
...
private struct CountBadge: View {
    ...
    var body: some View {
        ...
        .padding(.vertical, 7)
        .background(isSelected ? color.opacity(0.24) : .black.opacity(0.62), in: Capsule())
        .overlay(
            Capsule()
                .stroke(isSelected ? color.opacity(0.9) : .clear, lineWidth: 1)
        )
    }
}
...
private struct DirectionModeButtonStyle: ButtonStyle {
    let tint: Color
    let isSelected: Bool
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .padding(.vertical, 12)
            .background(
                isSelected ? tint.opacity(configuration.isPressed ? 0.78 : 0.92) : .white.opacity(configuration.isPressed ? 0.15 : 0.08),
