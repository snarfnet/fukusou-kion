...
final class CameraCounterViewModel: NSObject, ObservableObject {
    ...
    @Published var statusText = "入店計測中"
    @Published var selectedDirection: CountDirection = .in
    ...
    func reset() {
        ...
        recentEvents = []
        tracks = []
        statusText = isRunning ? modeStatusText : "\u{30EA}\u{30BB}\u{30C3}\u{30C8}\u{6E08}\u{307F}"
    }

    func selectDirection(_ direction: CountDirection) {
    ...
    func adjustSelected(amount: Int) {
        adjust(selectedDirection, amount: amount)
    }
    ...
    private func configureSession() {
        ...
        isCameraReady = true
        statusText = isRunning ? modeStatusText : "\u{958B}\u{59CB}\u{3067}\u{304D}\u{307E}\u{3059}"
        if isRunning {
            startSessionIfNeeded()
        }
    }
    ...
    private func handleDetections(_ observations: [VNHumanObservation]) {
        ...
        DispatchQueue.main.async {
            ...
            if self.isRunning {
                self.statusText = normalizedDetections.isEmpty ? self.modeStatusText : "\(self.selectedDirection.displayName) \(normalizedDetections.count)人検出中"
            }
        }
    }
    ...
    private func countCrossingIfNeeded(previousX: CGFloat, currentX: CGFloat, trackIndex: Int) {
        ...
        if crossedLeftToRight || crossedRightToLeft {
            ...
            DispatchQueue.main.async {
                self.adjust(self.selectedDirection, amount: 1)
            }
        }
    }

    private var modeStatusText: String {
        "\(selectedDirection.displayName)計測中"
    }
