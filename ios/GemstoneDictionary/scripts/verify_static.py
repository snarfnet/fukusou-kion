...
def main() -> int:
    ...
    results.append(check("判定履歴保存", "StoredScanHistoryEntry" in content and "UserDefaults.standard" in content and "loadScanHistory()" in content and "saveScanHistory()" in content and "JSONEncoder" in content and "JSONDecoder" in content))
    results.append(check("判定結果共有", "ShareLink" in content and "shareText(for candidate" in content and "shareText(for entry" in content and "天然石判定メモ" in content and "square.and.arrow.up" in content))
